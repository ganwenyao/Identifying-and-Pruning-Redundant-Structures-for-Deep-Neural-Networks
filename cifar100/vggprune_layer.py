...
parser.add_argument('--dropT', type=float, default=0.012, metavar='LR',
                    help='')
...
while conv_id >= 0:
    ...
    conv_id_end = len(conv_in_cfg_list) - 1
    cfg.pop(cfg_id)
    # if the last layer is maxpooling, remove it and expand the avgpooling size
    while cfg[-1] == 'M':
        cfg.pop()
    # print('prune_layer', cfg, conv_id, conv_id_end)
    newmodel = vgg(dataset=args.dataset, cfg=cfg)
    newmodel = newmodel.to(device)
    preserve, prune, linear = prune_layer(newmodel, conv_id, conv_id_end, model, args.freeze_conv, args.freeze_linear)
    ...
    print('(last_prec1 - prec): %.4f' % (last_prec1 - prec))
    if prec > args.pruneT and (last_prec1 - prec) <= args.dropT:
        ...
        for i, c in enumerate(cfg):
            if c != 'M':
                conv_in_cfg_list.append(i)
        conv_id_end = len(conv_in_cfg_list) - 1
        print('\nSuccess to prune layer_%d:' % (conv_id+1), cfg, '\n')
    else:
        success_flag_list.append(0)
        cfg = old_cfg
        print('Fail to prune layer_%d:' % (conv_id+1), 'back to:', old_cfg)
        del newmodel
    conv_id -= 1
