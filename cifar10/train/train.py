...
def train_fp16(model, optimizer, epoch, device, train_loader, valid, valid_size, log_interval):
    end = time.time()
    model.train()
    avg_loss = 0.
    train_acc = 0.
    ...
    if valid:
        train_len = train_len * (1-valid_size)
...
def train(model, optimizer, epoch, device, train_loader, valid, valid_size, log_interval):
    end = time.time()
    model.train()
    avg_loss = 0.
    train_acc = 0.
...
def valid(model, device, valid_loader, valid_size):
    model.eval()
    valid_loss = 0
    correct = 0
    valid_len = len(valid_loader.dataset) * valid_size
...
def train_eval(model, device, train_loader, valid_size, valid=True):
    model.eval()
    loss = 0
    correct = 0
    train_len = len(train_loader.dataset)
